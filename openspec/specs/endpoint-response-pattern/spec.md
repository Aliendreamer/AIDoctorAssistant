# Spec: Endpoint Response Pattern

## Purpose

Defines how FastEndpoints endpoint implementations must send HTTP responses, ensuring correct use of `Send.*` property methods so that the FastEndpoints request pipeline is properly terminated after each response.
